...
from radar.date_storage import (
...
def test_snapshot_database_copies_duckdb_to_dated_dir(tmp_path: Path) -> None:
    ...
    assert target == tmp_path / "data" / "snapshots" / "2026-03-13" / "radar_data.duckdb"
...
def test_cleanup_dated_reports_supports_both_name_patterns(tmp_path: Path) -> None:
    ...
    report_dir.mkdir()
    (report_dir / "game_20260301.html").write_text("old", encoding="utf-8")
    (report_dir / "2026-03-02.html").write_text("old-iso", encoding="utf-8")
    (report_dir / "game_20260312.html").write_text("new", encoding="utf-8")
    ...
    assert removed == 2
    assert not (report_dir / "game_20260301.html").exists()
    assert not (report_dir / "2026-03-02.html").exists()
    assert (report_dir / "game_20260312.html").exists()
...
def test_apply_date_storage_policy_combines_snapshot_and_cleanup(tmp_path: Path) -> None:
    ...
    (report_dir / "game_20250101.html").write_text("old", encoding="utf-8")
